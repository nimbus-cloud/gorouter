...
func (r *AccessLogRecord) makeRecord() string {
	statusCode, responseTime, appId, dest, extraHeaders := "-", "-", "-", "-", ""
	...
	return fmt.Sprintf(`%s - [%s] "%s %s %s" %s %d %d "%s" "%s" %s x_forwarded_for:"%s" x_forwarded_proto:"%s" vcap_request_id:%s response_time:%s app_id:%s dest:%s%s`+"\n",
		r.Request.Host,
		r.FormatStartedAt(),
		r.Request.Method,
